...
| 12 | ~~branch-picker 会话树~~ | **已完成(2026-07-17 A1)**:快照带分支点,`fromBranchableArray` + sibling 占位,BranchPicker 原生点亮,切换经 navigateTree 真实换路径;顺带修复 SSE 改为挂载期常驻(idle 广播不再丢) | - | - |
...
- A1 已知限制:分支序号恒显示当前为 1/N(占位插入序,非树时间序);切到"以未回复 user 消息结尾"的分支时 leaf 停其 parent(文本在树里不丢,composer 回填是后续升级);`hideWhenSingleBranch` 在 0.14.26 会把多分支也藏掉,已用 AuiIf branchCount 门控替代;vite HMR 可能使 globalThis 缓存的共享流失效(硬刷新即愈,仅开发态)。
