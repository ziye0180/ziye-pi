...
> update_time: 2026-07-16 23:56 CST
...
| # | 能力 | pi 侧 | 断点 | 工作量 |
|---|---|---|---|---|
| 1 | session-stats 面板 | `getSessionStats()` 完整(tokens+cost+contextUsage) | PiClient 契约无方法 | 一条 GET 路由四层直通,最轻 |
| 2 | 原生导出 html/jsonl | `exportToHtml/exportToJsonl` | 没接;现 /export 导投影 JSON | 单端点 |
| 3 | 手动 /compact | `compact(customInstructions)` 双通 | 事件只读镜像,不能触发 | 单端点 + slash 命令 |
| 4 | 侧栏按最近活跃排序 | SessionInfo 含时间 | `mapThreadMetadata` 没映射 lastMessageAt | 字段映射 |
| 5 | archive 持久化 | n/a | **实现级缺陷**:归档是 supervisor 内存 Set,bridge 重启全丢 | 落盘 sidecar + 启动回读 |
...
- archive 内存态缺陷(上表 #5)在做归档相关功能前必须先修,否则一切归档 UI 都是假状态。
